...
```mermaid
flowchart TB
    subgraph Phase1["Phase 1: Alarm Evaluation"]
        A[/"Device sends<br/>Telemetry/Attributes"/] --> B["TbDeviceProfileNode<br/><i>onMsg()</i>"]
        B --> C["DeviceState<br/><i>process()</i>"]
        C --> D["DataSnapshot<br/><i>Current values</i>"]
        C --> E["AlarmState<br/><i>createOrClearAlarms()</i>"]
        E --> F["AlarmRuleState<br/><i>eval()</i>"]
        F -->|Condition TRUE| G["calculateAlarmResult()"]
        G --> H["createDetails()"]
        H --> I["resolveAttributePatterns()<br/>🔧 <b>CUSTOMIZATION #2</b>"]
        I --> J[("Alarm Created<br/>in Database")]
    end

    subgraph Phase2["Phase 2: Notification Processing"]
        J --> K["DefaultNotificationRuleProcessor<br/><i>process()</i>"]
        K --> L["AlarmTriggerProcessor<br/><i>matchesFilter()</i>"]
        L --> M["constructNotificationInfo()<br/>🔧 <b>CUSTOMIZATION #1</b>"]
        M --> N["AlarmNotificationInfo"]
        N --> O["getTemplateData()<br/>🔧 <b>Flattens alarmDetails</b>"]
    end

    subgraph Phase3["Phase 3: Template Rendering"]
        O --> P["NotificationProcessingContext<br/><i>getProcessedTemplate()</i>"]
        P --> Q["TemplateUtils<br/><i>processTemplate()</i>"]
        Q --> R{{"Delivery Channel"}}
        R --> S["📧 Email"]
        R --> T["📱 SMS"]
        R --> U["💬 Slack"]
        R --> V["🔔 Mobile Push"]
    ...
    subgraph DataFlow["Data Flow"]
        D -.->|"${key}"| H
        DYN["DynamicPredicateValueCtx"] -.->|"${ss:attr}"| I
        DYN -.->|"Customer/Tenant<br/>Attributes"| I
    end

    style I fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
    style M fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
    style O fill:#e1f5fe,stroke:#0288d1,stroke-width:2px
```
